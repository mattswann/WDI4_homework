...
​/*Write a function called ```whatDoesYourStartupDo``` that displays the message in
 the console "So....your startup is basically like " + randomThis() + " for " + randomThat() + "?"*/

function randomThis(){
  var i = Math.floor(Math.random()*thisArray.length);
  var this = thisArray[i];
  return this;
}
function randomThat(){
  var r = parseint((Math.random()*10),10);
  var that = thatArray[r];
  return that;
}
