...
// returns random element from thisArray
function randomThis() {
	var index = Math.floor(Math.random() * thisArray.length);
	var thisChoice = thisArray[index];
	return thisChoice; 

}

// returns random element from thatArray
function randomThat() {
	var index = Math.floor(Math.random() * thatArray.length);
	var thatChoice = thatArray[index];
	return thatChoice; 
}
